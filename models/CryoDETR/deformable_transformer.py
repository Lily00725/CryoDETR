...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300, look_forward_twice=False, mixed_selection=False, use_ms_detr=False, use_aux_ffn=True, hybrid_encoder_layers=2, std_attn_level=-1):
        super().__init__()
        ...
        # encoder (AugDETR HAE): only the first `hybrid_encoder_layers` encoder layers use hybrid attention;
        # the remaining layers fall back to vanilla MSDeformAttn encoder layers.
        if hybrid_encoder_layers > 0:
            encoder_layers = []
            for layer_id in range(num_encoder_layers):
                if layer_id < hybrid_encoder_layers:
                    encoder_layers.append(HybridAttentionEncoderLayer(
                        d_model, dim_feedforward, dropout, activation,
                        num_feature_levels, nhead, enc_n_points,
                        std_attn_level=std_attn_level,
                    ))
                else:
                    encoder_layers.append(DeformableTransformerEncoderLayer(
                        d_model, dim_feedforward, dropout, activation,
                        num_feature_levels, nhead, enc_n_points,
                    ))
...
class HybridAttentionEncoderLayer(nn.Module):
    '''
    Hybrid Attention Encoder (HAE) layer (AugDETR-style):
      (1) Standard Multi-Head Self-Attention (MHA) + FFN on one selected feature level only (std_attn_level)
      (2) Replace the updated tokens back into the multi-scale sequence
      (3) MSDeformAttn + FFN on all scales (baseline Deformable DETR encoder)

    This layer does NOT include GRA/logit residual.
    '''
    def __init__(self,
                 d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, std_attn_level=-1):
        super().__init__()
        self.std_attn_level = std_attn_level

        # ---- standard self-attention on one selected level ----
        ...
        # FFN after standard self-attention (P5 branch)
        self.linear1_std = nn.Linear(d_model, d_ffn)
        self.activation_std = _get_activation_fn(activation)
        ...
        # ---- deformable self-attention on all scales ----
        self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)

        # FFN after deformable attention (main branch)
        self.linear1 = nn.Linear(d_model, d_ffn)
        self.activation = _get_activation_fn(activation)
        self.dropout2 = nn.Dropout(dropout)
        self.linear2 = nn.Linear(d_ffn, d_model)
    ...
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, padding_mask=None):
        # ---- 1) Slice one selected level for dense self-attention ----
        n_levels = int(level_start_index.shape[0])
        lvl = int(self.std_attn_level)
        if lvl < 0:
            lvl = n_levels + lvl
        lvl = max(0, min(lvl, n_levels - 1))

        start = int(level_start_index[lvl].item())
        end = int(level_start_index[lvl + 1].item()) if (lvl + 1) < n_levels else int(src.shape[1])

        tgt_src = src[:, start:end]
        tgt_pos = None if pos is None else pos[:, start:end]
        tgt_kpm = None if padding_mask is None else padding_mask[:, start:end]

        # ---- 2) Standard MHA on the selected level ----
        q = k = self.with_pos_embed(
            tgt_src.transpose(0, 1),
            None if tgt_pos is None else tgt_pos.transpose(0, 1)
        )
        v = tgt_src.transpose(0, 1)
        attn = self.std_self_attn(q, k, v, key_padding_mask=tgt_kpm)[0].transpose(0, 1)

        tgt_src = self.norm_std(tgt_src + self.dropout_std(attn))
        tgt_src = self.forward_ffn_std(tgt_src)

        # ---- 3) Replace the updated tokens back into the multi-scale sequence ----
        src = torch.cat((src[:, :start], tgt_src, src[:, end:]), dim=1)
        if pos is not None:
            pos = torch.cat((pos[:, :start], tgt_pos, pos[:, end:]), dim=1)
        # ---- 4) MSDeformAttn on all scales (baseline) ----
...
class DeformableTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers):
        super().__init__()
        if isinstance(encoder_layer, (list, tuple, nn.ModuleList)):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                ...
                # print(intermediate_o2m)          # 查看O2M中间输出的形状
...
def build_deforamble_transformer(args):
    return DeformableTransformer(
        ...
        two_stage_num_proposals=args.topk_eval,
        ...
        hybrid_encoder_layers=getattr(args, "hybrid_encoder_layers", 2),
        #std_attn_level=getattr(args, "std_attn_level", -1),
        std_attn_level=getattr(args, "std_attn_level", -1),
    )
